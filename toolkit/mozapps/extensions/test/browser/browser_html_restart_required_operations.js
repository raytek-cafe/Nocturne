...
async function assertCardMessage(win, addon, messageId) {
  ...
  await TestUtils.waitForCondition(() => {
    ...
    return (
      !messageBar.hidden &&
      id === messageId &&
      (args?.addon ?? args?.name) === addon.name
    );
...
add_task(async function test_restart_required_operation_messages() {
  ...
  await assertCardMessage(win, updateAddon, "details-notification-unsigned2");
  ...
  await assertCardMessage(win, enabledAddon, "details-notification-unsigned2");
  ...
  await assertCardMessage(win, disabledAddon, "details-notification-unsigned2");
