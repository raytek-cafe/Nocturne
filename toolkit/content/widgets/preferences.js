...
"use strict"; {
    ...
    document.addEventListener("select", e => {
        if (e.target.localName == "colorpicker") {
            let pane = e.target.closest("prefpane");
            if (pane)
                pane.userChangedValue(e.target);
        }
    });
    ...
    document.addEventListener("paneload", e => {
        ...
        for (var i = 0; i < elements.length; ++i) {
            try {
                ...
                Cu.reportError("*** No preference found for " +
    ...
    class MozPrefWindow extends MozXULElement {
        ...
        _init() {
            ...
            var panes = Array.from(this.children).filter(
                    n => n.localName === "prefpane");
            if (!panes.length)
                return;
    ...
    return false;
        ...
        _selectPane(aPaneElement) {
            ...
            var prefpanes = Array.from(this.preferencePanes);
            for (var i = 0; i < prefpanes.length; ++i) {
                if (prefpanes[i] === aPaneElement) {
                    this._paneDeck.selectedIndex = i;
                    prefpanes[i].selected = true;
                } else {
                    prefpanes[i].selected = false;
                }
            }
