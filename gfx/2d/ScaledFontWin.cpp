...
void StoreFontData(const uint8_t* aData, uint32_t aLength, uint32_t aIndex,
                   void* aBaton) {
  auto* data = static_cast<FontData*>(aBaton);
  data->mData.assign(aData, aData + aLength);
  data->mIndex = aIndex;
}
...
SkTypeface* ScaledFontWin::CreateSkTypeface() {
  FontData fontData;
  if (mUnscaledFont &&
      mUnscaledFont->GetFontFileData(StoreFontData, &fontData) &&
      !fontData.mData.empty()) {
    sk_sp<SkFontMgr> fontMgr = SkFontMgr_New_GDI();
    if (fontMgr) {
      sk_sp<SkTypeface> typeface = fontMgr->makeFromData(
          SkData::MakeWithCopy(fontData.mData.data(), fontData.mData.size()),
          fontData.mIndex);
      if (typeface) {
