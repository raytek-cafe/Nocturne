...
#include <algorithm>
namespace mozilla {
uint32_t VP9BitReader::ReadBits(int n) {
  if (n < 0 || n > 32) {
    mError = true;
    return 0;
  }
...
int32_t VP9BitReader::ReadSignedBits(int n) {
  int32_t v = (int32_t)ReadBits(n);
  int32_t sign = (int32_t)ReadBit();
  return sign ? -v : v;
}

static void ReadColorConfig(VP9BitReader& br, uint8_t profile,
                            VP9FrameHeader& h) {
  if (profile >= 2) {
    br.ReadBit();
  }
...
static uint32_t ReadFrameSize(VP9BitReader& br) { return br.ReadBits(16) + 1; }

void VP9HeaderParser::ParseLoopFilter(VP9BitReader& br, VP9FrameHeader& h) {
  ...
  for (int i = 0; i < 4; i++) {
    h.refDeltas[i] = mRefDeltas[i];
  }
  for (int i = 0; i < 2; i++) {
    h.modeDeltas[i] = mModeDeltas[i];
  }
}

void VP9HeaderParser::ParseQuantization(VP9BitReader& br, VP9FrameHeader& h) {
  ...
  h.lossless = (h.baseQIndex == 0 && h.deltaQYDc == 0 && h.deltaQUvDc == 0 &&
                h.deltaQUvAc == 0);
}

void VP9HeaderParser::ParseSegmentation(VP9BitReader& br, VP9FrameHeader& h) {
  ...
  if (h.segmentationUpdateMap) {
    for (int i = 0; i < 7; i++) {
      h.segmentationTreeProbs[i] = br.ReadBit() ? (uint8_t)br.ReadBits(8) : 255;
    }
    ...
    if (h.segmentationTemporalUpdate) {
      for (int i = 0; i < 3; i++) {
        h.segmentationPredProbs[i] =
            br.ReadBit() ? (uint8_t)br.ReadBits(8) : 255;
      }
    }
  }
...
void VP9HeaderParser::ParseTileInfo(VP9BitReader& br, VP9FrameHeader& h) {
...
bool VP9HeaderParser::ParseInternal(const uint8_t* aData, uint32_t aSize,
  ...
  memcpy(aHeader.segFeatureEnabled, mPrevious.segFeatureEnabled,
         sizeof(aHeader.segFeatureEnabled));
  memcpy(aHeader.segFeatureData, mPrevious.segFeatureData,
         sizeof(aHeader.segFeatureData));
  memcpy(aHeader.segmentationTreeProbs, mPrevious.segmentationTreeProbs,
         sizeof(aHeader.segmentationTreeProbs));
  memcpy(aHeader.segmentationPredProbs, mPrevious.segmentationPredProbs,
         sizeof(aHeader.segmentationPredProbs));
  ...
  if (aHeader.frameType == 0) {
    ...
    if (aHeader.colorSpace == 7 || aHeader.colorSpace == 6) {
      return false;
    }
    ...
    if (intraOnly) {
      ...
      for (int i = 0; i < 3; ++i) {
        if (!mRefFrameWidth[aHeader.refFrameIdx[i]]) {
          return false;
        }
      }
      bool foundRef = false;
  ...
  aHeader.usePrevFrameMvs = !aHeader.isIntra && !aHeader.errorResilientMode &&
                            !mPrevious.isIntra && mPrevious.showFrame &&
                            mPrevious.frameWidth == aHeader.frameWidth &&
  ...
  return !br.Failed() && aHeader.compressedHeaderSize != 0 &&
         ...
         aHeader.compressedHeaderSize <
             aSize - aHeader.uncompressedHeaderSizeBytes;
}

void VP9HeaderParser::Reset() {
  memset(&mPrevious, 0, sizeof(mPrevious));
  memset(mPrevious.segmentationTreeProbs, 255,
         sizeof(mPrevious.segmentationTreeProbs));
  memset(mPrevious.segmentationPredProbs, 255,
         sizeof(mPrevious.segmentationPredProbs));
...
bool VP9HeaderParser::Parse(const uint8_t* data, uint32_t size,
...
bool VP9SplitSuperframe(const uint8_t* data, uint32_t size,
                        uint32_t (&offsets)[8], uint32_t (&sizes)[8],
                        uint32_t& count) {
  count = 0;
  if (!data || !size) {
    return false;
  ...
  for (uint32_t i = 0; i < frames; ++i) {
    uint32_t length = 0;
    for (uint32_t j = 0; j < magnitude; ++j) {
      length |= uint32_t(data[pos++]) << (j * 8);
    }
    if (!length || length > size - indexSize - total) {
      return false;
    }
    offsets[i] = total;
    sizes[i] = length;
